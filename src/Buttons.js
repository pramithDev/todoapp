...
const Buttons = (props) => {
    return (
        <span className="tools">
            {props.completed ?
                <span className="icon complete disabled" onClick={props.taskMarkComplete}>
                    <Button style={complete} disabled >Complete</Button>
                </span>:
                <span className="icon complete" onClick={props.taskMarkComplete}>
                    <Button style={complete}>Complete</Button>
                </span>
            }

            {props.isEditing ?
                <span className="icon tick" onClick={props.taskToggleIsEditingAt}>
                    <FontAwesomeIcon icon={faCheckCircle} color="#4BB543" />
                </span>:
                <span className={props.completed ? 'icon edit disabled' : 'icon edit' }
                      onClick={ props.completed ? undefined : props.taskToggleIsEditingAt}
                >
                    <FontAwesomeIcon icon={faPencilAlt}/>
                </span>
            }

            <span className="icon delete" onClick={props.taskHandleRemove}>
